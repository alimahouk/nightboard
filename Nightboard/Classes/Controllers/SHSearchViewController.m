...
- (void)searchForQuery:(NSString *)query
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/search", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        [HUD hide:YES];
        ...
        if ( errorCode == 0 )
        {
            posts = [[responseObject objectForKey:@"response"] mutableCopy];
