...
- (void)createBoard
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/createboard", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        int errorCode = [[responseObject objectForKey:@"errorCode"] intValue];
        
        if ( errorCode == 0 )
        {
            NSString *boardID = [NSString stringWithFormat:@"%@", [responseObject objectForKey:@"response"]];
