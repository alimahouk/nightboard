...
- (void)post
{
    ...
    NSDictionary *parameters = @{@"token": appDelegate.SHToken,
                                 ...
                                 @"board_id": _boardID,
                                 @"color": [NSString stringWithFormat:@"%d", postColor],
                                 @"text": post};
    
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/createboardpost", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
