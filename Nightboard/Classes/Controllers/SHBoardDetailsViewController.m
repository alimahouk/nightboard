...
- (void)updateBoardInfo
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/updateboardinfo", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)leaveBoard
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/leaveboard", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        int errorCode = [[responseObject objectForKey:@"errorCode"] intValue];
        ...
        if ( errorCode == 0 )
        {
            ...
            if ( _isLastMember )
            {
                [appDelegate.mainView removeBoard:_boardID];
                [appDelegate.mainView refreshCloud];
...
- (void)uploadCover
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/boardcoverupload", SH_DOMAIN] parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        if ( imageData )
        {
...
- (void)removeCurrentCover
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/boardcoverremove", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        int errorCode = [[responseObject objectForKey:@"errorCode"] intValue];
