...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [appDelegate.locationManager updateLocation];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    appDelegate.mainView.shouldEnterFullscreen = YES;
}
...
- (void)postStatus
{
    ...
    {
        [doneButton setTitle:NSLocalizedString(@"GENERIC_DONE", nil)];
    ...
    else if ( status || keyboardIsShown )
    {
        ...
        [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/updatestatus", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
            int errorCode = [[responseObject objectForKey:@"errorCode"] intValue];
            
            if ( errorCode == 0 )
            {
                NSMutableDictionary *statusData = [[responseObject objectForKey:@"response"] mutableCopy];
                ...
                //[appDelegate.mainView refreshMiniFeed]; // Refresh the feed.
...
- (void)userDidTapAndHoldRow:(UILongPressGestureRecognizer *)longPress
{
    if ( longPress.state == UIGestureRecognizerStateBegan )
    {
        ...
        if ( !statusList.editing )
        {
            [doneButton setTitle:NSLocalizedString(@"GENERIC_SAVE", nil)];
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ( !(IS_IOS7) )
    {
        currentLocationLabel.frame = CGRectMake(20, 30 + abs(MAX(29.5, statusEditorTextSize.height) - 29.5), 280, currentLocationTextSize.height + currentLocationLabel.firstLineIndent);
    }
    else
    {
        currentLocationLabel.frame = CGRectMake(20, 20 + abs(MAX(29.5, statusEditorTextSize.height) - 29.5), 280, currentLocationTextSize.height + currentLocationLabel.firstLineIndent);
    }
