...
- (void)loadView
{
    ...
    cover.image = appDelegate.mainView.wallpaper.image;
    cover.alpha = 0.7;
    cover.opaque = YES;
...
- (void)joinBoard
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/joinboard", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
...
- (void)loadBoardBatch:(int)batchNo
{
    ...
    [manager POST:[NSString stringWithFormat:@"http://%@/theboard/api/getboardinfo", SH_DOMAIN] parameters:parameters success:^(AFHTTPRequestOperation *operation, id responseObject) {
        int errorCode = [[responseObject objectForKey:@"errorCode"] intValue];
...
- (void)loadCoverPhoto
{
    ...
    NSURL *URL = [NSURL URLWithString:[NSString stringWithFormat:@"http://%@/userphotos/theboard/boards/%@/photos/f_%@.jpg", SH_DOMAIN, _boardID, coverHash]];
